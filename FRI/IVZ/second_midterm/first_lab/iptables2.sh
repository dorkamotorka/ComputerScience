...
iptables -A INPUT -p tcp --match multiport --dport 22,80,443 -m state --state NEW -j ACCEPT
iptables -A OUTPUT -p tcp --match multiport --dport 22,80,443 -m state --state NEW -j ACCEPT
...
iptables -A FORWARD -p tcp -m multiport --ports 22,80,443 -m state --state NEW -j ACCEPT
