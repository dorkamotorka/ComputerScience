        ...
        env.add(new Agent("bob") {
            ...
            public void task() throws Exception {
                ...
                for (int i = 0 ; i < 10 ; i++) {
                    ...
                    //print("IV received is '%s'", hex(iv));
                    decrypt.init(Cipher.DECRYPT_MODE, key, new IvParameterSpec(iv));
                    final byte[] dt = decrypt.doFinal(received);
                    System.out.println("Received from Alice: " + new String(dt));

                    final byte[] payload = "Hey Alice, kisses.".getBytes();
                    final byte[] iv1 = new byte[16];
                    new SecureRandom().nextBytes(iv1);;
                    encrypt.init(Cipher.ENCRYPT_MODE, key, new IvParameterSpec(iv1));
                    final byte[] cipherText = encrypt.doFinal(payload);
